/**
 * Sync Bears Player Headshots Script
 *
 * This script updates the bears_players table in the datalab Supabase
 * with ESPN headshot URLs based on the roster from sportsmockery.com
 *
 * Run with: npx tsx scripts/sync-bears-headshots.ts
 */
...
if (!DATALAB_SERVICE_KEY) {
  console.error('Error: DATALAB_SUPABASE_SERVICE_ROLE_KEY environment variable is required')
  console.log('Set it with: export DATALAB_SUPABASE_SERVICE_ROLE_KEY="your-key"')
  process.exit(1)
}

const datalabAdmin = createClient(DATALAB_URL, DATALAB_SERVICE_KEY, {
  auth: {
...
const BEARS_ROSTER: { espnId: string; name: string; number?: string }[] = [
  // Offense (27)
...
async function syncHeadshots() {
  console.log('🏈 Bears Headshot Sync Script')
  console.log('==============================')
  console.log(`Roster size: ${BEARS_ROSTER.length} players`)
  ...
  for (const rosterPlayer of BEARS_ROSTER) {
    ...
    if (existingPlayer) {
      ...
      if (updateError) {
        ...
        console.log(`  ❌ ${rosterPlayer.name}: ${updateError.message}`)
      } else {
        results.updated.push(rosterPlayer.name)
        console.log(`  ✅ ${rosterPlayer.name} - updated`)
      }
    } else {
      // Player not in database - could insert if needed
      results.skipped.push(`${rosterPlayer.name} (not in database)`)
      console.log(`  ⏭️  ${rosterPlayer.name} - not in database`)
    }
  }
  ...
  if (results.skipped.length > 0) {
    console.log('')
    console.log('Skipped players (not in database):')
    results.skipped.forEach(s => console.log(`  - ${s}`))
  }
}

// Run the sync
syncHeadshots().catch(console.error)
