          ...
          {games.map((game) => {
            ...
            return (
              <button key={game.gameId} onClick={() => setSelectedGameId(game.gameId)}
                ...
                style={{
                  backgroundColor: isSelected ? '#27251F' : 'var(--sm-card)',
                  border: isSelected ? '1px solid #C4CED4' : '1px solid var(--sm-border)',
                  ...(isSelected ? { boxShadow: '0 0 0 3px rgba(196, 206, 212, 0.3)' } : {}),
                }}>
                ...
                <div className="text-left">
                  ...
                  <div className="font-semibold" style={{ color: isSelected ? '#FAFAFB' : 'var(--sm-text)' }}>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm font-semibold" style={{ color: isSelected ? '#FAFAFB' : 'var(--sm-text)' }}>{game.teamScore}-{game.oppScore}</span>
                  </div>
                </div>
              </button>
            )
        ...
        <div className="space-y-6">
          ...
          <div className="flex rounded-xl overflow-hidden" style={{ border: '1px solid var(--sm-border)' }}>
            <button onClick={() => setActiveTeam('whitesox')}
              ...
              style={{
                backgroundColor: activeTeam === 'whitesox' ? '#27251F' : 'var(--sm-card)',
                color: activeTeam === 'whitesox' ? '#FAFAFB' : 'var(--sm-text-muted)',
              }}>
            ...
            <button onClick={() => setActiveTeam('opponent')}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 font-semibold transition-all hover:brightness-95 dark:hover:brightness-110"
              style={{
                backgroundColor: activeTeam === 'opponent' ? '#27251F' : 'var(--sm-card)',
            ...
            <div className="flex" style={{ borderBottom: '1px solid var(--sm-border)' }}>
              {(['batting', 'pitching'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  ...
                  style={{
                    backgroundColor: activeTab === tab ? '#C4CED4' : 'transparent',
                    color: activeTab === tab ? '#27251F' : 'var(--sm-text-muted)',
                  }}>
                  {tab}
                </button>
              ))}
          ...
          <GameHighlights
            ...
            homeTeam={boxScore.whitesox.isHome ? 'whitesox' : boxScore.opponent.abbrev}
            awayTeam={boxScore.whitesox.isHome ? boxScore.opponent.abbrev : 'whitesox'}
        ...
        <tbody>
          {players.map((p, i) => (
            <tr key={i} className="last:border-0 transition-all hover:brightness-95 dark:hover:brightness-110" style={{ borderBottom: '1px solid var(--sm-border)' }}>
              ...
              <td className="px-2 py-3 text-center font-bold text-sm" style={{ color: 'var(--sm-text)' }}>{p.hr ?? '-'}</td>
