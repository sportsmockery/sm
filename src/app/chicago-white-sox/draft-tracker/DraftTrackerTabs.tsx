      ...
      {activeTab === 'mock-draft' && (
        <div
          ...
          style={{
            marginBottom: '32px',
            padding: '28px',
            background: 'linear-gradient(135deg, rgba(39,37,31,0.4), rgba(196,206,212,0.1))',
        ...
        >
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
            <div>
              ...
              >
                Build Your White Sox Mock Draft
              </h2>
              <p style={{ color: 'var(--sm-text-muted)', fontSize: '14px', margin: 0 }}>
                Use our interactive mock draft tool to build your ideal White Sox draft board for the rebuild.
              </p>
            </div>
            <Link
              href="/mock-draft"
      ...
      {activeTab === 'prospects' && (
        <section style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
            {[
              { position: 'SP', priority: 'High', note: 'Ace for rotation' },
              { position: 'SS', priority: 'High', note: 'Franchise shortstop' },
              { position: 'OF', priority: 'Medium', note: 'Corner outfield bat' },
              { position: 'C', priority: 'Medium', note: 'Future backstop' },
              { position: 'RP', priority: 'Low', note: 'Bullpen depth' },
              { position: 'UTIL', priority: 'Low', note: 'Versatile bat' },
            ].map((need) => (
              <div
                key={need.position}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {hubItems.map((item) => (
                  <div key={item.id} className="glass-card glass-card-sm" style={{ position: 'relative' }}>
                    ...
                    <h3 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--sm-text)', margin: '0 0 6px 0', lineHeight: 1.3 }}>
                      {item.headline}
                    </h3>
          ...
          <section>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {displayPosts.map((post) => {
                const href = post.categorySlug ? `/${post.categorySlug}/${post.slug}` : `/chicago-white-sox/${post.slug}`
                return (
                  <Link key={post.id} href={href} style={{ textDecoration: 'none', display: 'block' }}>
                    <article className="glass-card glass-card-sm" style={{ display: 'flex', gap: '16px', overflow: 'hidden' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <h3 style={{ color: 'var(--sm-text)', fontSize: '15px', fontWeight: 600, lineHeight: 1.4, margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {post.title}
                        </h3>
