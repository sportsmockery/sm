...
interface Data {
  ...
  writers: Array<{ id: number; name: string; avatar: string | null; email: string; role: string; posts: number; views: number; avgViews: number; topCategories: string[]; avgReadTime: number; avgScore: number }>
    ...
    <div className="space-y-5 pb-8">
      ...
      {loading ? <Skeleton/> : data ? <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          ...
          <MiniKPI label="Avg Read" value={`${data.overview.avgReadTime}m`} sub="minutes" color="#dc2626" icon={I.clk} />
        </div>
...
function OverviewPanel({d}:{d:Data}) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid lg:grid-cols-3 gap-4">
      ...
      <Card title="Views Distribution"><div style={{height:220}}>
        <Bar data={{labels:d.viewsDistribution.map(x=>x.range),datasets:[{data:d.viewsDistribution.map(x=>x.count),backgroundColor:['#ef4444cc','#f59e0bcc','#2563ebcc','#059669cc','#7c3aedcc'],borderRadius:4}]}} options={bOpts(false,true)} />
      </div></Card>
    </div>
    ...
    <div className="grid lg:grid-cols-12 gap-4">
      ...
      <div className="lg:col-span-4 space-y-4">
        <Card title="Read Time Distribution"><div style={{height:160}}>
          <Bar data={{labels:d.readTimeDistribution.map(x=>x.range),datasets:[{data:d.readTimeDistribution.map(x=>x.count),backgroundColor:['#06b6d4cc','#2563ebcc','#7c3aedcc','#d97706cc','#dc2626cc'],borderRadius:3}]}} options={bOpts(false,true)} />
        </div></Card>
        <Card title="Quality Scores"><div style={{height:160}}>
          <Doughnut data={{labels:d.scoreDistribution.map(x=>x.range),datasets:[{data:d.scoreDistribution.map(x=>x.count),backgroundColor:['#ef4444','#f59e0b','#2563eb','#059669','#94a3b8'],borderColor:'#fff',borderWidth:2}]}} options={dOpts('bottom')} />
        </div></Card>
      </div>
    </div>
  </div>
...
function WritersPanel({d}:{d:Data}) {
  ...
  return <div className="space-y-4">
    ...
    <Card title="Full Leaderboard">
      ...
      <div className="overflow-x-auto"><table className="w-full">
        <thead><tr className="border-b border-slate-100">{['#','Writer','Posts','Views','Avg Views/Post','Read Time','Score','Categories'].map(h=><th key={h} className="text-left px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">{h}</th>)}</tr></thead>
        <tbody>{sorted.map((w,i)=><tr key={w.id} className="border-b border-slate-50 hover:bg-slate-50/50">
          ...
          <td className="px-2 py-2 text-xs text-slate-500 tabular-nums">{w.avgReadTime}m</td>
          <td className="px-2 py-2">{w.avgScore > 0 ? <span className="text-xs font-bold" style={{color:w.avgScore>=70?'#059669':w.avgScore>=40?'#d97706':'#dc2626'}}>{w.avgScore}</span> : <span className="text-[10px] text-slate-300">—</span>}</td>
...
function ContentPanel({d}:{d:Data}) {
  return <div className="space-y-4">
      ...
      <div className="lg:col-span-4 space-y-4">
        <Card title="Topic Breakdown">
          <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">
            {d.topicBreakdown.slice(0,12).map((t,i)=><ProgressBar key={t.topic} label={t.topic} value={t.count} max={d.topicBreakdown[0]?.count||1} color={P[i%P.length]}/>)}
            {d.topicBreakdown.length===0 && <p className="text-xs text-slate-400 text-center py-4">No topic data</p>}
          </div>
        </Card>
        <Card title="Read Time"><div style={{height:160}}>
          <Bar data={{labels:d.readTimeDistribution.map(x=>x.range),datasets:[{data:d.readTimeDistribution.map(x=>x.count),backgroundColor:['#06b6d4cc','#2563ebcc','#7c3aedcc','#d97706cc','#dc2626cc'],borderRadius:3}]}} options={bOpts(false,true)} />
        </div></Card>
      </div>
    </div>
    ...
    {/* Charts row */}
    <div className="grid lg:grid-cols-3 gap-4">
      <Card title="Category Avg Views"><div style={{height:240}}>
        <Bar data={{labels:d.categories.slice(0,8).map(c=>c.name),datasets:[{data:d.categories.slice(0,8).map(c=>c.avgViews),backgroundColor:P.slice(0,8).map(c=>c+'cc'),borderRadius:4}]}} options={{...bOpts(true,true)}} />
      </div></Card>
      <Card title="Quality Score Distribution"><div style={{height:240}}>
        <Doughnut data={{labels:d.scoreDistribution.map(x=>x.range),datasets:[{data:d.scoreDistribution.map(x=>x.count),backgroundColor:['#ef4444','#f59e0b','#2563eb','#059669','#94a3b8'],borderColor:'#fff',borderWidth:2}]}} options={dOpts('bottom')} />
      </div></Card>
      <Card title="Views by Article"><div style={{height:240}}>
        <Bar data={{labels:d.topContent.slice(0,6).map(p=>p.title.substring(0,25)+'...'),datasets:[{data:d.topContent.slice(0,6).map(p=>p.views||0),backgroundColor:P.slice(0,6).map(c=>c+'cc'),borderColor:P.slice(0,6),borderWidth:1,borderRadius:4}]}} options={bOpts(true,true)} />
      </div></Card>
    </div>
  ...
  return <div className="space-y-4">
    <div className="grid lg:grid-cols-3 gap-4">
      <Card title="Domain Overview">
        ...
        <StatRow label="Avg Quality Score" value={d.overview.avgScore > 0 ? String(d.overview.avgScore)+'/100' : 'N/A'} color={d.overview.avgScore>=70?'#059669':'#d97706'}/>
      </Card>
      <Card title="Content Health">
        <div className="space-y-3">
          ...
          <div><p className="text-[10px] text-slate-400 mb-1">Avg Views Per Article</p><div className="flex items-center gap-2"><div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full rounded-full bg-purple-500" style={{width:`${Math.min(100,d.overview.avgViews/10*100)}%`}}/></div><span className="text-xs font-bold text-slate-700">{fN(d.overview.avgViews)}</span></div></div>
          <div><p className="text-[10px] text-slate-400 mb-1">Avg Read Time</p><div className="flex items-center gap-2"><div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full rounded-full bg-amber-500" style={{width:`${Math.min(100,d.overview.avgReadTime/15*100)}%`}}/></div><span className="text-xs font-bold text-slate-700">{d.overview.avgReadTime} min</span></div></div>
