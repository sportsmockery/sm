...
export async function GET(
  ...
  try {
    ...
    const [soxResult, oppResult] = await Promise.all([
      datalabAdmin
        ...
        .select(`player_id, ${BATTING_COLS}, ${PITCHING_COLS}, is_opponent,
          whitesox_players!inner(name, position, headshot_url)`)
    ...
    const transform = (s: any, isOpp: boolean) => ({
      name: isOpp ? (s.opponent_player_name || 'Unknown') : (s.whitesox_players?.name || 'Unknown'),
      position: isOpp ? (s.opponent_player_position || '') : (s.whitesox_players?.position || ''),
      headshotUrl: isOpp ? (s.opponent_player_headshot_url || null) : (s.whitesox_players?.headshot_url || null),
