...
export const metadata: Metadata = {
  title: 'Chicago Bulls Salary Cap 2026 | Sports Mockery',
  description:
    'Live cap space, contracts, luxury tax analysis for the Chicago Bulls.',
  openGraph: {
    title: 'Chicago Bulls Salary Cap 2026',
    description: 'Live cap space, contracts, luxury tax analysis.',
    type: 'website',
  },
  twitter: {
    title: 'Chicago Bulls Salary Cap 2026',
...
function getPositionGroup(pos: string): string {
  ...
  const centers = ['C']
  if (guards.includes(pos)) return 'Guards'
  if (forwards.includes(pos)) return 'Forwards'
  if (centers.includes(pos)) return 'Centers'
  return 'Other'
}
...
export default async function BullsCapTrackerPage() {
    ...
    buildSafeFetch(
      async () => {
        ...
        return {
          ...
          headshots: (headshotsResult.data || []) as { espn_player_id: string; headshot_url: string }[],
        }
      },
      { cap: null, rows: [] as ContractRow[], headshots: [] as { espn_player_id: string; headshot_url: string }[] },
      { label: 'bulls cap tracker data' }
    ),
  ])
  ...
  for (const p of capData.headshots) {
    if (p.espn_player_id && p.headshot_url) headshotMap.set(String(p.espn_player_id), p.headshot_url)
  }
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
                  ...
                  >
                    <span
                      style={{
                        ...
                        fontSize: '13px',
                        fontWeight: 700,
                        color: idx === 0 ? '#CE1141' : 'var(--sm-text-dim)',
                        flexShrink: 0,
                        textAlign: 'center',
                      }}
                    >
        ...
        >
          ...
          <p style={{ color: 'var(--sm-text-muted)', fontSize: '14px', margin: '0 0 16px 0' }}>
            Get AI-powered insights on Bulls cap space, potential trades, and free agency targets.
          </p>
          <Link
