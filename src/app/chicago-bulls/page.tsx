      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '32px', maxWidth: '1320px', margin: '0 auto' }}>
        ...
        <div className="hub-grid-bulls" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '32px' }}>
            ...
            <section>
              <style>{`
                ...
                .bulls-feature-card::before {
                  ...
                  bottom: 0;
                  width: 3px;
                  background: linear-gradient(180deg, #CE1141, #000000);
                  opacity: 0.7;
                  transition: opacity 0.4s;
                }
                .bulls-feature-card:hover {
                  transform: translateX(4px);
                  border-color: rgba(206, 17, 65, 0.2);
                  background: var(--sm-card-hover);
                }
                .bulls-feature-card:hover::before {
                  opacity: 1;
                }
                .bulls-feature-btn {
                  display: inline-flex;
              ...
              `}</style>
              <div className="bulls-features-grid">
                <TeamFeatureCard
                  href="/chicago-bulls/trade-rumors"
                  ...
                  title="Trade Rumors"
                  description="Live Bulls trade tracker and rumor mill."
                  buttonText="Latest Rumors"
                  accentColor="#CE1141"
                />
                <TeamFeatureCard
                  href="/chicago-bulls/draft-tracker"
                  ...
                  title="Draft Tracker"
                  description="2026 NBA draft prospects, mock drafts, and trade scenarios."
                  buttonText="Mock Draft"
                  accentColor="#CE1141"
                />
                <TeamFeatureCard
                  href="/chicago-bulls/cap-tracker"
                  ...
                  title="Salary Cap"
                  description="Live cap space, contracts, trade simulator."
                  buttonText="Cap Moves"
                  accentColor="#CE1141"
                />
                <TeamFeatureCard
                  href="/chicago-bulls/depth-chart"
                  ...
                  title="Depth Chart"
                  description="Interactive roster with starters and backups."
                  buttonText="Roster"
                  accentColor="#CE1141"
                />
                <TeamFeatureCard
                  href="/chicago-bulls/game-center"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <QuickLinksCard slug="chicago-bulls" />
      ...
      <Link href={href} className="group" style={{ textDecoration: 'none', display: 'block' }}>
        <article className="glass-card" style={{ overflow: 'hidden', padding: 0 }}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <style>{`@media (min-width: 768px) { .bulls-hero-row { flex-direction: row !important; } .bulls-hero-img { width: 50% !important; } }`}</style>
            <div className="bulls-hero-row" style={{ display: 'flex', flexDirection: 'column' }}>
              {post.featuredImage && (
                <div className="bulls-hero-img" style={{ position: 'relative', aspectRatio: '16/9', overflow: 'hidden', width: '100%' }}>
                  <Image src={post.featuredImage} alt="" fill style={{ objectFit: 'cover', transition: 'transform 0.3s' }} priority />
                </div>
              )}
              <div style={{ padding: '24px', flex: 1 }}>
                <span className="sm-tag" style={{ marginBottom: '12px', display: 'inline-block' }}>Bulls</span>
                <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", color: 'var(--sm-text)', fontSize: '20px', fontWeight: 700, lineHeight: 1.3, margin: '0 0 12px 0', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                  {post.title}
                </h3>
                {post.excerpt && (
                  <p style={{ color: 'var(--sm-text-muted)', fontSize: '15px', lineHeight: 1.6, margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{post.excerpt}</p>
                )}
...
function TeamFeatureCard({
  ...
  return (
    <Link href={href} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="bulls-feature-card">
        <div
          style={{
            width: '40px',
            height: '40px',
  ...
  return (
    <div className="glass-card glass-card-static">
      ...
      <div style={{ textAlign: 'center', padding: '16px 0', borderTop: '1px solid var(--sm-border)' }}>
        <div style={{ fontSize: '36px', fontWeight: 700, color: 'var(--sm-text)', fontFamily: "'Space Grotesk', sans-serif" }}>
          {record ? `${record.wins}-${record.losses}` : '--'}
        </div>
        <div style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--sm-text-dim)', fontWeight: 600, marginTop: '4px' }}>
          Record
        </div>
      </div>
    </div>
...
function QuickLinksCard({ slug }: { slug: string }) {
  ...
  return (
    <div className="glass-card glass-card-static">
      <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", color: 'var(--sm-text)', fontWeight: 700, fontSize: '16px', margin: '0 0 16px 0' }}>
        Quick Links
      </h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {links.map((link) => (
          ...
          >
            ...
            <svg width="16" height="16" style={{ color: 'var(--sm-text-dim)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="glass-card glass-card-static">
      ...
      <Link
        ...
        style={{ display: 'block', width: '100%', textAlign: 'center', textDecoration: 'none', borderRadius: 'var(--sm-radius-pill)' }}
      >
        Ask Scout
      </Link>
    </div>
...
function FanChatWidget({ teamLabel, channel }: { teamLabel: string; channel: string }) {
  return (
    <div className="glass-card glass-card-static">
      ...
      <p style={{ color: 'var(--sm-text-muted)', fontSize: '14px', margin: '0 0 16px 0' }}>Join the conversation with fellow {teamLabel} fans.</p>
      <Link
