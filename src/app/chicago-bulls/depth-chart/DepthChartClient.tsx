...
export default function DepthChartClient({
  ...
  // Bulls has one group "Starters" with positions PG/SG/SF/PF/C
  // Filter by position within that group
  const filteredGroups = positionGroups.map((g) => ({
    ...g,
    positions: g.positions.filter((p) => posFilter === 'all' || p.position === posFilter),
      ...
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            {(['all', 'PG', 'SG', 'SF', 'PF', 'C'] as PosFilter[]).map((f) => (
              <button
                key={f}
                onClick={() => setPosFilter(f)}
                className={`draft-tab${posFilter === f ? ' active' : ''}`}
              >
                {f === 'all' ? 'All' : f}
              </button>
            ))}
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {group.positions.map((pos, posIdx) => (
                <div key={pos.position}>
                  ...
                  >
                    {pos.position === 'PG' ? 'Point Guard' : pos.position === 'SG' ? 'Shooting Guard' : pos.position === 'SF' ? 'Small Forward' : pos.position === 'PF' ? 'Power Forward' : 'Center'}
                  </div>
                        ...
                        >
                          ...
                          <div
                            style={{
                              ...
                              fontSize: '12px',
                              fontWeight: 700,
                              color: player.isStarter ? teamColor : 'var(--sm-text-dim)',
                              textAlign: 'center',
                              flexShrink: 0,
                            }}
                          >
