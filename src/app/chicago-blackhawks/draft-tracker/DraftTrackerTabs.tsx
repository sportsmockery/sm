        ...
        >
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
            <div>
              ...
              >
                Build Your Blackhawks Mock Draft
              </h2>
              <p style={{ color: 'var(--sm-text-muted)', fontSize: '14px', margin: 0 }}>
                Use our interactive mock draft tool to build your ideal Blackhawks draft board.
              </p>
            </div>
            <Link
              href="/mock-draft"
      ...
      {activeTab === 'prospects' && (
        <section style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
            {[
              { position: 'Center', priority: 'High', note: 'Build behind Bedard' },
              { position: 'Defenseman', priority: 'High', note: 'Blue line depth' },
              { position: 'Goalie', priority: 'Medium', note: 'Future starter' },
              { position: 'Winger', priority: 'Medium', note: 'Scoring depth' },
              { position: 'Power Forward', priority: 'Low', note: 'Physical presence' },
              { position: 'Depth', priority: 'Low', note: 'Pipeline depth' },
            ].map((need) => (
              <div
                key={need.position}
          ...
          <section>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {displayPosts.map((post) => {
                const href = post.categorySlug ? `/${post.categorySlug}/${post.slug}` : `/chicago-blackhawks/${post.slug}`
                return (
