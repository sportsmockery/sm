...
type SideFilter = 'all' | 'forwards' | 'defense' | 'goalies'
...
export default function DepthChartClient({
  ...
  const filteredGroups = positionGroups.filter((g) => {
    ...
    if (sideFilter === 'forwards') return g.key === 'forwards'
    if (sideFilter === 'defense') return g.key === 'defensemen'
    if (sideFilter === 'goalies') return g.key === 'goalies'
    return true
  })
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--sm-text-muted)', fontSize: '16px', margin: 0, lineHeight: 1.6 }}>
          Interactive roster with forwards, defensemen, and goaltenders. Click any player for their full profile.
        </p>
      </div>
      ...
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', fontSize: '14px' }}>
            ...
            <span>
              <span style={{ color: 'var(--sm-text-muted)' }}>FWD: </span>
              <span style={{ fontWeight: 600, color: 'var(--sm-text)' }}>{forwardsCount}</span>
            </span>
            <span>
            ...
            <span>
              <span style={{ color: 'var(--sm-text-muted)' }}>G: </span>
              <span style={{ fontWeight: 600, color: 'var(--sm-text)' }}>{goalieCount}</span>
            </span>
          </div>
          <div style={{ display: 'flex', gap: '6px' }}>
            {(['all', 'forwards', 'defense', 'goalies'] as SideFilter[]).map((f) => (
              <button
                key={f}
                ...
                style={{
                  ...
                  backgroundColor: sideFilter === f ? '#CF0A2C' : 'transparent',
                  color: sideFilter === f ? '#ffffff' : 'var(--sm-text-muted)',
                  borderColor: sideFilter === f ? '#CF0A2C' : 'var(--sm-border)',
                }}
              >
                {f === 'all' ? 'All' : f === 'defense' ? 'Defense' : f === 'goalies' ? 'Goalies' : 'Forwards'}
              </button>
            ))}
          </div>
        </div>
                    ...
                    >
                      ...
                      <div
                        style={{
                          ...
                          fontSize: '12px',
                          fontWeight: 700,
                          color: isStarter ? '#CF0A2C' : 'var(--sm-text-dim)',
                          textAlign: 'center',
                          flexShrink: 0,
                        }}
                      >
                      ...
                      <div
                        style={{
                          ...
                          fontWeight: 700,
                          flexShrink: 0,
                          backgroundColor: isStarter ? '#CF0A2C' : 'var(--sm-surface)',
                      ...
                      <div
                        style={{
                          ...
                          overflow: 'hidden',
                          flexShrink: 0,
                          border: isStarter ? '2px solid #CF0A2C' : '2px solid var(--sm-border)',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: '12px', color: 'var(--sm-text-muted)' }}>
                          ...
                          {player.birthCountry && <span> &middot; {player.birthCountry}</span>}
                        </div>
                      </div>
                      ...
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        style={{
                          color: isHovered ? '#CF0A2C' : 'var(--sm-text-dim)',
                          flexShrink: 0,
