...
export const metadata: Metadata = {
  title: 'Chicago Blackhawks Salary Cap 2026 | Sports Mockery',
  description:
    'Live cap space, contracts, spending breakdown.',
  openGraph: {
    title: 'Chicago Blackhawks Salary Cap 2026',
    description: 'Live cap space, contracts, spending breakdown.',
    type: 'website',
  },
  twitter: {
    title: 'Chicago Blackhawks Salary Cap 2026',
...
function getPositionGroup(pos: string): string {
  const fwd = ['C', 'LW', 'RW', 'F', 'W']
  const def = ['D', 'LD', 'RD']
  const goalie = ['G']
  if (fwd.includes(pos)) return 'Forwards'
  if (def.includes(pos)) return 'Defensemen'
  if (goalie.includes(pos)) return 'Goaltenders'
  return 'Other'
}
...
const CONTRACT_TYPE_COLORS: Record<string, { bg: string; text: string }> = {
  'Entry-Level': { bg: 'rgba(0,212,255,0.15)', text: '#00D4FF' },
  'Standard': { bg: 'rgba(214,176,94,0.15)', text: '#D6B05E' },
  'Bridge': { bg: 'rgba(34,197,94,0.15)', text: '#16a34a' },
  'Extension': { bg: 'rgba(34,197,94,0.15)', text: '#16a34a' },
  '35+': { bg: 'rgba(188,0,0,0.15)', text: '#BC0000' },
  'Buyout': { bg: 'rgba(107,114,128,0.15)', text: '#6b7280' },
}
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <p style={{ color: 'var(--sm-text-muted)', fontSize: '16px', margin: 0, lineHeight: 1.6 }}>
            Live cap space, contract breakdowns, and position spending for the 2025-26 season.
          </p>
        </div>
        ...
        {/* Hub Updates Feed */}
        <HubUpdatesFeed hubSlug="cap-tracker" teamSlug="chicago-blackhawks" title="Cap Moves" emptyState="No cap updates yet." />
                  ...
                  >
                    <span
                      style={{
                        ...
                        fontSize: '13px',
                        fontWeight: 700,
                        color: idx === 0 ? '#CF0A2C' : 'var(--sm-text-dim)',
                        flexShrink: 0,
                        textAlign: 'center',
                      }}
                    >
        ...
        >
          ...
          <p style={{ color: 'var(--sm-text-muted)', fontSize: '14px', margin: '0 0 16px 0' }}>
            Get AI-powered insights on Blackhawks cap space, potential moves, and free agency targets.
          </p>
          <Link
