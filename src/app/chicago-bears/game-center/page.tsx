...
export default async function BearsGameCenterPage() {
  const team = CHICAGO_TEAMS.bears

  const [separatedRecord, nextGame, lastGame, schedule] = await Promise.all([
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <p style={{ color: 'var(--sm-text-muted)', fontSize: '16px', margin: 0, lineHeight: 1.6 }}>
            Live scores, play-by-play, and game previews for every Bears game.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '32px' }}>
          {[
            { label: 'Full Schedule', href: '/chicago-bears/schedule' },
            { label: 'Box Scores', href: '/chicago-bears/scores' },
            { label: 'Team Stats', href: '/chicago-bears/stats' },
        ...
        >
          ...
          <Link
            href="/scout-ai?team=chicago-bears&q=How%20did%20the%20Bears%20do%20in%20their%20last%20game"
...
function GameRow({ game }: { game: BearsGame }) {
  ...
  return (
    <Link
      href={isCompleted ? '/chicago-bears/scores' : '/chicago-bears/schedule'}
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: '10px', color: 'var(--sm-text-dim)' }}>
            {game.isPlayoff ? 'Playoff' : game.isPreseason ? 'Pre' : `Wk ${game.week}`}
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {game.venue && (
            <div style={{ fontSize: '11px', color: 'var(--sm-text-dim)' }}>{game.venue}</div>
          )}
        </div>
