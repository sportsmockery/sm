  ...
  return (
    <div className={className}>
      <div className="rounded-2xl p-6" style={{ backgroundColor: 'var(--sm-card)', border: '1px solid var(--sm-border)' }}>
        ...
        {/* Question */}
        <p className="mb-4 font-semibold" style={{ color: 'var(--sm-text)' }}>
          {poll.question}
        </p>
          ...
          {votes.map((option) => {
            ...
            return (
              <button
                ...
                `}
                style={{ borderColor: selectedOption === option.id ? '#8B0000' : 'var(--sm-border)' }}
              >
                {/* Progress bar (shown after voting) */}
                {hasVoted && (
                  <div
                    className="absolute inset-0 bg-gradient-to-r from-[#8B0000]/10 to-transparent"
        ...
        {!hasVoted && (
          <button
            ...
            className="mt-4 w-full rounded-lg px-4 py-2 font-semibold text-white transition-colors disabled:cursor-not-allowed disabled:opacity-50"
