...
interface BearsAIHudProps {
  activePlayer: BearsPlayer | null
}

export default function BearsAIHud({ activePlayer }: BearsAIHudProps) {
  ...
  const [scoutMessages, setScoutMessages] = useState<{ role: string; content: string }[]>([
    {
      role: 'assistant',
      content: "Bear Down! I'm seeing a lot of chatter on the radar about the offensive line. Next-Gen just flagged a trend you might want to see\u2014should we dive in?",
    },
  ])
  ...
  const submitScout = async () => {
    ...
    const raw = scoutQuery.trim()
    // Prepend Bears context
    const context = activePlayer
      ? `[Context: Bears Obsidian Hub, viewing ${activePlayer.fullName}] `
      : '[Context: Bears Obsidian Hub, viewing roster] '
    setScoutQuery('')
    setScoutMessages(prev => [...prev, { role: 'user', content: raw }])
    setScoutLoading(true)
    try {
      const res = await fetch('/api/ask-ai', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: context + raw, sessionId: sessionIdRef.current }),
      })
        ...
        {activePanel === 'scout' && (
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 9, color: '#bc0000', fontWeight: 600 }}>
                BEARS INTEL
              </span>
            </div>
            ...
            >
              <input
                ...
                placeholder="Ask Scout about the Bears..."
                style={{
                  flex: 1,
                  padding: '8px 12px',
                  fontSize: 13,
        ...
        {activePanel === 'gm' && (
          ...
          >
            ...
            {briefingError && !briefingLoading && (
              <div style={{ textAlign: 'center', padding: '20px 0', fontSize: 12, color: isDark ? '#555' : '#999' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {briefing.map((item, i) => (
                  ...
                  >
                    ...
                    {item.stat && (
                      <span style={{ fontSize: 11, color: '#bc0000', fontWeight: 600 }}>
                        {item.stat}
                      </span>
                    )}
                  </div>
