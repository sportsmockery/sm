...
function PerformanceScore({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <div className="w-8 h-1 rounded-full" style={{ backgroundColor: 'rgba(11,15,20,0.08)' }}>
...
export default function LeaderboardsPanel({ leaders }: Props) {
  ...
  return (
    ...
    >
      <div className="px-4 py-2.5 border-b" style={{ borderColor: 'rgba(11,15,20,0.08)' }}>
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium" style={{ color: '#0B0F14' }}>{player.name}</span>
              ...
              <span className="text-xs" style={{ color: 'rgba(11,15,20,0.5)' }}>{player.headline}</span>
            </div>
            <div className="flex flex-col items-end gap-1 ml-3">
              <span className="text-sm font-bold" style={{ color: '#00D4FF' }}>{player.primary_stat}</span>
              <div className="flex gap-2">
                {player.secondary_stats.map((s, i) => (
                  <span key={i} className="text-xs" style={{ color: 'rgba(11,15,20,0.4)' }}>{s}</span>
                ))}
              </div>
        ...
        {activeTab === 'struggling' && leaders.players.struggling.map((player) => (
          ...
          >
            <div className="flex-1 min-w-0">
              ...
              <span className="text-xs block" style={{ color: 'rgba(11,15,20,0.5)' }}>{player.headline}</span>
            </div>
            <div className="flex flex-col items-end gap-1 ml-3">
        ...
        {activeTab === 'best_units' && leaders.units.best.map((unit, i) => (
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium" style={{ color: '#0B0F14' }}>{unit.unit_label}</span>
            ...
            <p className="text-xs mt-1" style={{ color: 'rgba(11,15,20,0.5)' }}>{unit.summary}</p>
          </div>
        ))}
