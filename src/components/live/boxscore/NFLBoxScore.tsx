    ...
    return (
      <div className="rounded-lg overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.15)' }}>
        ...
        <div className="p-4 space-y-4">
          {passers.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>Passing</h4>
              {passers.map(p => (
                <div key={p.player_id} className="flex justify-between py-1">
                  <span style={{ color: '#FAFAFB' }}>{p.full_name}</span>
                  <span style={{ color: 'rgba(255,255,255,0.5)' }}>{p.nfl_pass_completions}/{p.nfl_pass_attempts}, {p.nfl_passing_yards} YDS, {p.nfl_passing_tds} TD, {p.nfl_interceptions} INT</span>
                </div>
              ))}
            </div>
          )}
          {rushers.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>Rushing</h4>
              {rushers.map(p => (
                <div key={p.player_id} className="flex justify-between py-1">
                  <span style={{ color: '#FAFAFB' }}>{p.full_name}</span>
                  <span style={{ color: 'rgba(255,255,255,0.5)' }}>{p.nfl_rush_attempts} CAR, {p.nfl_rushing_yards} YDS, {p.nfl_rushing_tds} TD</span>
                </div>
              ))}
            </div>
          )}
          {receivers.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}>Receiving</h4>
              {receivers.map(p => (
                <div key={p.player_id} className="flex justify-between py-1">
                  <span style={{ color: '#FAFAFB' }}>{p.full_name}</span>
                  <span style={{ color: 'rgba(255,255,255,0.5)' }}>{p.nfl_receptions} REC, {p.nfl_receiving_yards} YDS, {p.nfl_receiving_tds} TD</span>
                </div>
              ))}
            </div>
          )}
