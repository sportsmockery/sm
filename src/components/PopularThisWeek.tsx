      ...
      <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
        {posts.map((post, index) => (
          <Link
            ...
            className="group flex items-start gap-4 px-6 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
          >
            {/* Rank number */}
            <div className="shrink-0 w-8 h-8 rounded-lg bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center font-bold text-zinc-500 dark:text-zinc-400 text-sm font-[var(--font-montserrat)]">
              {index + 1}
            </div>

            {/* Content */}
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm leading-snug line-clamp-2 group-hover:text-[#8B0000] dark:group-hover:text-[#FF6666] transition-colors">
                {post.title}
              </h4>
