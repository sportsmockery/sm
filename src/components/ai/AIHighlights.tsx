...
export default function AIHighlights({ teamSlug, teamName, className = '' }: AIHighlightsProps) {
  ...
  if (loading) {
    return (
      <div className={`bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden ${className}`}>
    ...
    return (
      <div className={`bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden ${className}`}>
        <div className="px-4 py-3 border-b border-red-600">
          <h3 className={`text-black dark:text-white text-lg ${montserrat.className}`}>
            AI Highlights
          </h3>
        </div>
  ...
  return (
    ...
    >
      ...
      {/* Footer */}
      <footer className="px-4 py-3 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800">
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <h4 className={`text-sm font-medium text-black dark:text-white line-clamp-2 ${montserrat.className}`}>
            {highlight.title}
          </h4>
        </div>
...
function HighlightContent({
  ...
  switch (highlight.type) {
    ...
    default:
      return <p className="text-zinc-600 dark:text-zinc-400">{String(highlight.content)}</p>
  }
}
...
function ChartHighlight({
  ...
  return (
    <div className="h-48 bg-zinc-50 dark:bg-zinc-900 rounded-lg p-2">
      {data.chartType === 'bar' ? (
...
function DataHighlight({ content }: { content: string }) {
  return (
    <div className="bg-zinc-50 dark:bg-zinc-900 rounded-lg p-4">
      <p className={`text-2xl font-bold text-black dark:text-white ${montserrat.className}`}>
        {content}
      </p>
    </div>
  )
...
function CommentaryHighlight({ content }: { content: string }) {
  return (
    <blockquote className="border-l-4 border-red-600 pl-4 py-2 bg-zinc-50 dark:bg-zinc-900 rounded-r-lg">
      <p className="text-zinc-700 dark:text-zinc-300 italic font-serif">
        "{content}"
      </p>
