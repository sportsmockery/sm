...
// ECharts chart returned from DataLab generate-chart endpoint
interface EChartsChart {
...
/**
 * Hydrate serialized animation functions from DataLab.
 * DataLab serializes JS functions as strings (e.g., "(idx) => idx * 100")
 * since JSON can't carry functions. This converts them back.
 */
function hydrateAnimationFns(opt: Record<string, unknown>): Record<string, unknown> {
  const hydrate = (obj: Record<string, unknown>) => {
    for (const [k, v] of Object.entries(obj)) {
      if (typeof v === 'string' && v.startsWith('(idx)')) {
        try {
          obj[k] = new Function('return ' + v)()
        } catch { /* leave as string if invalid */ }
      } else if (v && typeof v === 'object' && !Array.isArray(v)) {
        hydrate(v as Record<string, unknown>)
      } else if (Array.isArray(v)) {
        v.forEach((item, i) => {
          if (item && typeof item === 'object') hydrate(item as Record<string, unknown>)
        })
      }
    }
  }
...
export default function PostIQChartGenerator({
  ...
  // ECharts mode: when DataLab returns ECharts options directly
  const [echartsCharts, setEchartsCharts] = useState<EChartsChart[]>([])
  const [echartsMode, setEchartsMode] = useState(false)
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // Handle ECharts format from DataLab generate-chart endpoint
        if (data.format === 'echarts' && data.charts?.length > 0) {
          const hydrated = data.charts.map((c: any) => ({
            options: hydrateAnimationFns(c.options),
            title: c.title || c.options?.title?.text || '',
          }))
          setEchartsCharts(hydrated)
          setEchartsMode(true)
          ...
          onAnalysisComplete?.({
            ...
            chartType: 'bar',
            chartTitle: hydrated[0]?.title || 'Chart',
            data: [],
            paragraphIndex: 1,
            reasoning: 'ECharts chart generated by DataLab',
          })
          return
        }

        // Legacy format: data points for D3/custom rendering
        setEchartsCharts([])
        setEchartsMode(false)
  ...
  const hasCharts = echartsMode ? echartsCharts.length > 0 : !!analysis
  const chartCount = echartsMode ? echartsCharts.length : allCharts.length
  const currentChartTitle = echartsMode
    ? (echartsCharts[selectedChartIndex]?.title || 'Chart')
    : (analysis?.chartTitle || '')

  return (
    <>
      {/* Success Indicator - Chart data detected */}
      {showIndicator && hasCharts && !showModal && (
          ...
          <div className="flex items-center gap-2 rounded-full bg-gradient-to-r from-purple-600 to-purple-700 pl-4 pr-2 py-2 shadow-lg shadow-purple-500/25">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-white">
                {chartCount > 1 ? `${chartCount} charts detected` : 'Chart data detected'}
              </span>
              {!echartsMode && analysis?.confidence && (
            ...
            {chartCount > 1 && (
              <div className="flex items-center gap-1">
                <button
                  onClick={() => {
                    const newIndex = (selectedChartIndex - 1 + chartCount) % chartCount
                    setSelectedChartIndex(newIndex)
                    if (!echartsMode) setAnalysis(allCharts[newIndex])
                  }}
                ...
                <span className="text-xs text-white/70">{selectedChartIndex + 1}/{chartCount}</span>
                <button
                  onClick={() => {
                    const newIndex = (selectedChartIndex + 1) % chartCount
                    setSelectedChartIndex(newIndex)
                    if (!echartsMode) setAnalysis(allCharts[newIndex])
                  }}
                  className="rounded-full p-1 text-white/70 hover:text-white hover:bg-white/10 transition-colors"
            ...
            <div className="absolute bottom-full mb-2 right-0 w-96 rounded-xl shadow-2xl overflow-hidden" style={{ backgroundColor: 'var(--sm-card)', border: '1px solid var(--sm-border)' }}>
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--sm-border)', backgroundColor: 'var(--sm-surface)' }}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold" style={{ color: 'var(--sm-text)' }}>{currentChartTitle}</p>
                    <p className="text-xs" style={{ color: 'var(--sm-text-muted)' }}>
                      {echartsMode ? 'ECharts' : `${analysis?.chartType} chart • ${analysis?.data.length} data points`}
                    </p>
                  </div>
              ...
              <div className="p-4">
                {echartsMode && echartsCharts[selectedChartIndex] ? (
                  <ReactECharts
                    option={{
                      ...echartsCharts[selectedChartIndex].options,
              ...
              {!echartsMode && analysis?.extractedFrom && (
