...
export default function NotificationBell() {
  ...
  const getTypeIcon = (type: Notification['type']) => {
    switch (type) {
      case 'breaking':
        return (
          <div className="flex h-8 w-8 items-center justify-center rounded-full" style={{ backgroundColor: 'rgba(239, 68, 68, 0.15)' }}>
            <svg className="h-4 w-4" style={{ color: '#dc2626' }} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      case 'score':
        return (
          <div className="flex h-8 w-8 items-center justify-center rounded-full" style={{ backgroundColor: 'rgba(16, 185, 129, 0.15)' }}>
            <svg className="h-4 w-4" style={{ color: '#059669' }} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      default:
        return (
          <div className="flex h-8 w-8 items-center justify-center rounded-full" style={{ backgroundColor: 'var(--sm-surface)' }}>
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" style={{ color: 'var(--sm-text-muted)' }}>
    ...
    <div ref={menuRef} className="relative">
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--sm-border)' }}>
          <span className="font-semibold" style={{ color: 'var(--sm-text)' }}>Notifications</span>
          {unreadCount > 0 && (
            <button
              onClick={markAllRead}
        ...
        <div className="max-h-80 overflow-y-auto">
          {notifications.length > 0 ? (
            ...
            <div className="px-4 py-8 text-center" style={{ color: 'var(--sm-text-muted)' }}>
              No notifications
            </div>
          )}
        </div>
        ...
        <div className="p-2" style={{ borderTop: '1px solid var(--sm-border)' }}>
          <Link
            href="/notifications"
            onClick={() => setIsOpen(false)}
            className="block rounded-lg px-3 py-2 text-center text-sm font-medium transition-colors"
            style={{ color: '#8B0000' }}
          >
