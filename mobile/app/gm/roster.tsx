...
export default function GMRosterScreen() {
  ...
  const positions = useMemo(() => {
    const pos = new Set(state.roster.map(p => p.position))
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [state.roster, search, posFilter])

  const isSelected = (p: PlayerData) =>
    state.selectedPlayers.some(s => s.player_id === p.player_id)
  ...
  const renderPlayer = ({ item }: { item: PlayerData }) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        onPress={() => dispatch({ type: 'TOGGLE_PLAYER', player: item })}
      >
        <View style={styles.playerRow}>
          {item.headshot_url ? (
            <Image source={{ uri: item.headshot_url }} style={styles.headshot} contentFit="cover" />
          ) : (
            <View style={[styles.headshot, styles.headshotPlaceholder, { backgroundColor: teamConfig?.color || '#ccc' }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <View style={styles.headerCenter}>
          <View style={styles.stepRow}>
            ...
            <Text style={[styles.headerTitle, { color: colors.text }]}>Select Players to Trade</Text>
          </View>
          <Text style={[styles.headerSubtitle, { color: colors.textMuted }]}>
            {teamConfig?.name || ''} · {state.selectedPlayers.length} selected
          </Text>
        </View>
      </View>
      ...
      {state.selectedPlayers.length > 0 && (
        <View style={[styles.footer, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
          <TouchableOpacity
            ...
            onPress={() => router.push('/gm/opponent')}
          >
            <Text style={styles.continueBtnText}>
              Continue with {state.selectedPlayers.length} player{state.selectedPlayers.length !== 1 ? 's' : ''}
            </Text>
